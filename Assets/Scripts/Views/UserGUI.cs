...
public class UserGUI : MonoBehaviour
{
    ...
    //主菜单界面
    void mainMenu() {
        //在指定位置绘制标签，显示文本为"打飞碟"，使用预定义的bigStyle样式
        GUI.Label(new Rect(Screen.width / 2 - menu_width * 0.5f, Screen.height * 0.1f, menu_width, menu_height), "趣味打飞碟", bigStyle);
        //在指定位置绘制按钮，显示文本为"开始游戏"，使用指定的位置和大小，返回一个布尔值表示按钮是否被点击
    ...
    void GameStart() {
        ...
        //用于返回游戏的得分
        GUI.Label(new Rect(0,0,150,50), "摧毁得分: " + score, smallStyle);
        //用于返回游戏的轮数
        GUI.Label(new Rect(0,60,150,50), "UFO波次: " + round, smallStyle);
        //在指定位置绘制按钮，显示文本为"难度升级"，使用指定的位置和大小，返回一个布尔值表示该按钮是否被点击
        if (GUI.Button(new Rect(0, 120, 150, 50), "难度升级"))
        {
            //如果该按钮被点击
